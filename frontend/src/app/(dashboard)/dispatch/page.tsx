    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Real-time multi-interval generation matching, spinning reserve verification, and curtailment tracking</p>
        </div>
        
        <div className="flex bg-slate-900 rounded-md border border-slate-700 p-1">
          <button className="px-3 py-1 text-xs font-medium text-slate-400 hover:text-white rounded">1h</button>
          <button className="px-3 py-1 text-xs font-medium text-slate-400 hover:text-white rounded">6h</button>
          <button className="px-3 py-1 text-xs font-medium text-slate-400 hover:text-white rounded">12h</button>
          <button className="px-3 py-1 text-xs font-medium bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded">24h</button>
        </div>
      </div>
      ...
      <Card className="bg-slate-900/50 border-slate-800">
        ...
        <CardContent className="pt-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          <div className="space-y-1">
            ...
            <div className="text-xl font-bold text-white">{formatPower(liveData.solar)}</div>
            <div className="text-[10px] text-slate-500">Cap: {formatPower(250000)}</div>
          </div>
          <div className="space-y-1 border-l border-slate-800 pl-4">
            ...
            <div className="text-xl font-bold text-white">{formatPower(liveData.wind)}</div>
            <div className="text-[10px] text-slate-500">Cap: {formatPower(100000)}</div>
          </div>
          <div className="space-y-1 border-l border-slate-800 pl-4">
            ...
            <div className="text-[10px] text-slate-500">Discharging</div>
          </div>
          <div className="space-y-1 border-l border-slate-800 pl-4">
            <div className="text-[10px] font-bold text-red-500 uppercase flex items-center gap-1"><Droplet size={10}/> Diesel Genset</div>
            <div className="text-xl font-bold text-white">{formatPower(liveData.diesel)}</div>
            <div className="text-[10px] text-slate-500">Cap: {formatPower(750000)}</div>
          </div>
          <div className="space-y-1 border-l border-slate-800 pl-4">
            <div className="text-[10px] font-bold text-purple-400 uppercase flex items-center gap-1"><Zap size={10}/> Total Demand</div>
            <div className="text-xl font-bold text-white">{formatPower(liveData.demand)}</div>
          ...
          <div className="space-y-1 border-l border-slate-800 pl-4">
            ...
            <div className="text-[10px] text-slate-500">N-1 Compliant</div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="bg-slate-900 border-slate-800">
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <div>
            <CardTitle className="text-sm font-bold text-white">Graph 1: Power Mix by Source (24h Interval)</CardTitle>
            <p className="text-[10px] text-slate-500 mt-1">Stacked generation (Solar, Wind, Diesel, Battery) vs Community Load Demand curve</p>
          </div>
          <div className="text-[10px] bg-slate-800 px-2 py-1 rounded text-emerald-400 font-mono">Panel Yield Opt: 99%</div>
        </CardHeader>
        <CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card className="bg-slate-900 border-slate-800">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <div>
              <CardTitle className="text-sm font-bold text-white">Graph 2: Load vs Generation Imbalance</CardTitle>
              <p className="text-[10px] text-slate-500 mt-1">Demand matched vs spinning reserves & unserved energy</p>
            </div>
        ...
        <Card className="bg-slate-900 border-slate-800">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <div>
              <CardTitle className="text-sm font-bold text-white">Graph 4: Renewable Curtailment Analysis</CardTitle>
              <p className="text-[10px] text-slate-500 mt-1">Potential vs Absorbed Energy (BESS charging limits)</p>
            </div>
            <div className="text-[10px] text-slate-500 font-mono">Curtailment Rate</div>
          </CardHeader>
          <CardContent>
            <div className="h-[200px] w-full mt-2">
              <ResponsiveContainer width="100%" height="100%">
      ...
      <Card className="bg-slate-900 border-slate-800">
        <CardHeader>
          <CardTitle className="text-sm font-bold text-white">Graph 2: Dispatch Interval Timeline</CardTitle>
          <p className="text-[10px] text-slate-500 mt-1">Dominant power supplier sequence determined by merit-order LP optimizer</p>
        </CardHeader>
        <CardContent>
